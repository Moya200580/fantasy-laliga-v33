...
def calcular_penalizacion_v3(
    ...
    # CERO LOOK-AHEAD BIAS: solo fechas estrictamente anteriores.
    jugador = jugador[
        jugador["fecha"] < fecha
    ].copy()
    ...
    apariciones = len(jugador)
    rentabilidades = jugador["rentabilidad_7d"].astype(float).values

    rentabilidad_media = np.mean(rentabilidades)
    acierto = np.mean(rentabilidades > 0) * 100

    # Menos del mínimo: no penalizamos por falta de muestra.
    ...
    # 1. FALLOS REALES
    # --------------------------------------------------------
    numero_fallos = int(np.sum(rentabilidades <= 0))
    porcentaje_fallos = numero_fallos / apariciones * 100

    # Solo penaliza el exceso de fallos sobre el 40%.
    componente_fallos = min(
        max(porcentaje_fallos - 40.0, 0.0) * 0.12,
        8.0
    )

    # --------------------------------------------------------
    # 2. RENTABILIDAD MEDIA NEGATIVA
    # --------------------------------------------------------
    componente_rentabilidad = 0.0

    if rentabilidad_media < 0:
        componente_rentabilidad = min(
            abs(rentabilidad_media) * 0.65,
            8.0
        )
    ...
    if apariciones >= 6:
        mitad = apariciones // 2
        antigua = np.mean(rentabilidades[:mitad])
        reciente = np.mean(rentabilidades[mitad:])
    ...
    # PENALIZACIÓN TOTAL
    #
    # NO existe ningún componente basado en apariciones.
    # Un jugador que aparece mucho y funciona bien no recibe
