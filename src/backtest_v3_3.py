...
# Fantasy LaLiga
#
# V3.3 - MEMORIA ADAPTATIVA BASADA EN FALLOS
#
...
# Mínimo de recomendaciones ANTERIORES necesarias para que
# la memoria pueda penalizar una nueva recomendación.
#
# Con 3 antecedentes, la 4ª aparición ya puede ser evaluada.
# Esto evita el error de dejar sin penalización precisamente
# la primera recomendación que ya tiene una muestra mínima.
#
# IMPORTANTE: este valor NO añade penalización por frecuencia.
# Solo define cuándo existe evidencia histórica suficiente.
...
print()
print("=" * 78)
print("🧪 BACKTEST DEL MODELO DE OPORTUNIDADES - V3.3")
print("=" * 78)
print()
...
def calcular_penalizacion_v3(
    ...
    """
    V3.3 - Penalizacion adaptativa basada exclusivamente en
    evidencia historica anterior.

    Reglas:
    - El numero de apariciones NO penaliza.
    - Se necesita un minimo de historial antes de penalizar.
    - Los fallos reales generan penalizacion.
    - Una racha de fallos aumenta la penalizacion.
    - Una rentabilidad media claramente negativa aumenta la penalizacion.
    - Un jugador que aparece mucho y funciona bien no se penaliza por frecuencia.
    - Un jugador con pocas apariciones pero evidencia muy mala puede ser penalizado
      una vez alcanzado el minimo de historial.
    - Solo se usan recomendaciones de fechas estrictamente anteriores.
    - Las rentabilidades futuras de la fecha actual nunca entran en su score.
    """
    ...
    # CERO LOOK-AHEAD BIAS: solo fechas anteriores.
    jugador = jugador[
        jugador["fecha"] < fecha
    ].copy()
    ...
    # Solo conservamos la ventana historica configurada.
    if len(jugador) > VENTANA_MEMORIA:
        jugador = jugador.tail(VENTANA_MEMORIA)

    apariciones = len(jugador)

    rentabilidades = pd.to_numeric(
        jugador["rentabilidad_7d"],
        errors="coerce"
    ).dropna()
    ...
    # Sin muestra suficiente: no penalizamos.
    if apariciones < MIN_APARICIONES_PENALIZACION:
        return {
            "penalizacion": 0.0,
            "apariciones": apariciones,
            "rentabilidad_media_7d": float(rentabilidad_media),
            "acierto_7d": float(acierto),
            "fallos_consecutivos": 0,
            "deterioro_reciente": 0.0,
        }
    ...
    # --------------------------------------------------------
    # 1. RENTABILIDAD MEDIA NEGATIVA
    # --------------------------------------------------------
    componente_rentabilidad = 0.0

    if rentabilidad_media < 0:
        componente_rentabilidad = min(
            abs(rentabilidad_media) * PESO_RENTABILIDAD_V3,
            MAX_RENTABILIDAD_V3
        )
    ...
    # 2. FALLOS REALES
    #
    # A diferencia de V3 anterior, aqui el porcentaje de fallos
    # SI aporta penalizacion. La frecuencia por si misma no.
    # --------------------------------------------------------
    componente_fallos = min(
        (porcentaje_fallos / 100) * MAX_FALLOS_V3,
        MAX_FALLOS_V3
    )
    ...
    # --------------------------------------------------------
    componente_racha = min(
        fallos_consecutivos * PESO_RACHA_FALLOS_V3,
        MAX_RACHA_V3
    )

    # --------------------------------------------------------
    # 4. DETERIORO RECIENTE
    # --------------------------------------------------------
    componente_deterioro = 0.0

    if deterioro_reciente > 2:
        componente_deterioro = min(
            deterioro_reciente * PESO_DETERIORO_V3,
            MAX_DETERIORO_V3
        )

    # --------------------------------------------------------
    # PROTECCION DE JUGADORES QUE FUNCIONAN BIEN
    #
    # Muchas apariciones NO generan penalizacion. Si la evidencia
    # historica es claramente buena, no añadimos penalizacion por
    # el mero hecho de haber aparecido muchas veces.
    # --------------------------------------------------------
    if (
        rentabilidad_media > 0
        and acierto >= UMBRAL_ACIERTO_BUENO
        and fallos_consecutivos == 0
    ):
        componente_rentabilidad = 0.0
        componente_deterioro = 0.0

        # Los fallos aislados pueden contar, pero limitamos su peso
        # para no convertir un historial globalmente bueno en malo.
        componente_fallos = min(
            componente_fallos,
            1.5
        )
    ...
    return {
        ...
        "rentabilidad_media_7d": float(rentabilidad_media),
        "acierto_7d": float(acierto),
        "fallos_consecutivos": int(fallos_consecutivos),
        "deterioro_reciente": float(deterioro_reciente),
    }
...
salida_problematicos = (
    DATA_DIR
    / "backtest_jugadores_problematicos_v3_3.csv"
)
...
salida_umbrales_v3 = (
    DATA_DIR
    / "backtest_v3_3_umbrales_score.csv"
)
...
print("=" * 78)
print("✅ BACKTEST ADAPTATIVO V3.3 COMPLETADO")
print("=" * 78)
print()
print("IMPORTANTE:")
print("La V3.3 NO penaliza por número de apariciones.")
print("La frecuencia solo determina si existe muestra suficiente para activar la memoria.")
